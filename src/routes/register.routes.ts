  ...
  postRegister() {
    return async (req: Request, res: Response) => {
      ...
      if ( body.type == 0 ) {
        ...
        let insertEmergency = `INSERT INTO Emergency (Patient_ID, Firstname, Lastname, Relation, Country, Postcode, Province, Subdistrict, District, Address, sameAddress, Email, Phone_No) VALUES(${Info.insertId}, '${body.emergency.first_name}', '${body.emergency.last_name}', '${body.emergency.relation}', ${emergencyAddress.Country}, '${emergencyAddress.Postcode ? emergencyAddress.Postcode : ''}', '${emergencyAddress.Province ? emergencyAddress.Province : ''}', '${emergencyAddress.Subdistrict}', '${emergencyAddress.District ? emergencyAddress.District : ''}', '${emergencyAddress.Address ? emergencyAddress.Address : ''}', ${body.emergency.sameAddress}, '${body.emergency.email}', '${body.emergency.phone_no}');`
        ...
        let insertFinan = `INSERT INTO Financial (Self_Pay, Company_Contact, Insurance, Patient_ID) VALUES(${_.indexOf(body.patient_info.payment_method, 'Self pay') >= 0 ? 0 : 1}, ${_.indexOf(body.patient_info.payment_method, 'Company contact') >= 0 ? 0 : 1}, ${_.indexOf(body.patient_info.payment_method, 'Insurance') >= 0 ? 0 : 1}, ${Info.insertId});`
